...
class MBR {
  ...
  constructor() {
    ...
    for( var i = 0; i < this.codeRegions; i++ ) {
      ...
      this.code.push( new MBR.Code( region.offset, region.length ) )
    }
 ...
 */
MBR.CODE_REGIONS = [
  { offset: 0x000, length: MBR.TABLE_OFFSET },
]
...
MBR.detectFormat = function( buffer, offset ) {
  ...
  } else if( buffer.readUIntBE( offset + 0x02, 6 ) === MBR.NewLoader.SIGNATURE ) { // 'NEWLDR'
    return 'NEWLDR'
...
MBR.parse = function( buffer, offset ) {
  ...
  switch( MBR.detectFormat( buffer, offset ) ) {
    case 'MODERN': mbr = new MBR.Modern(); break
    case 'NEWLDR': mbr = new MBR.NewLoader(); break
    case 'DISKMANAGER': mbr = new MBR.DiskManager(); break
    case 'AAP': mbr = new MBR.AAP(); break
    case 'AST': mbr = new MBR.AST(); break
    default: mbr = new MBR.Classic(); break
  }

  return mbr.parse( buffer, offset )
...
MBR.NewLoader = require( './format/newldr' )
