...
class MBR {
   ...
   * @returns {Array<MBR.Partition>}
   */
  getExtendedPartitions() {
  ...
  getEFIPart() {
    ...
    for( var i = 0; i < this.partitions.length; i++ ) {
      ...
      if( part.type === MBR.Partition.EFI_PROTECTIVE || part.type === MBR.Partition.EFI_HYBRID ) {
        return part
      }
    }
...
MBR.detectFormat = function( buffer, offset, noAssert = false ) {
  ...
  var isAST = buffer[ offset + 0x17C ] === 0x5A && buffer[ offset + 0x17D ] === 0xA5
  if( isAST ) return 'AST'

  var isDM = buffer[ offset + 0x0FC ] === 0xAA && buffer[ offset + 0x0FD ] === 0x55
  if( isDM ) return 'DISKMANAGER'

  var isNEWLDR = buffer.readUIntBE( offset + 0x02, 6 ) === MBR.NewLdr.SIGNATURE
  if( isNEWLDR ) return 'NEWLDR'

  var isAAP = buffer[ offset + 0x1AC ] === 0x78 && buffer[ offset + 0x1AD ] === 0x56
  if( isAAP ) return 'AAP'

  // TODO: Explain
  var isMODERN = (
    buffer.readUInt32LE( offset + 0x0DA ) === 0x00000000 &&
    buffer[ offset + 0x0DC ] >= 0x80
  ) || (
    buffer.readUInt32LE( offset + 0x1B8 ) !== 0 && (
      buffer.readUInt16LE( offset + 0x1BC ) === 0x0000 ||
      buffer.readUInt16LE( offset + 0x1BC ) === 0x5A5A
    )
  )
