...
AST.prototype = {
  ...
  get buffer() {
    ...
    this.partitions[4].write( buffer, offset )
    this.partitions[5].write( buffer, offset += 0x10 )
    this.partitions[6].write( buffer, offset += 0x10 )
    this.partitions[7].write( buffer, offset += 0x10 )

    // Write out primary partition records
    this.partitions[0].write( buffer, offset += 0x10 )
    this.partitions[1].write( buffer, offset += 0x10 )
    this.partitions[2].write( buffer, offset += 0x10 )
    this.partitions[3].write( buffer, offset += 0x10 )
  ...
  set buffer( value ) {
    ...
    for( i = 4; i < count; i++ ) {
      this.partitions[i].parse( value, offset + ( i * 0x10 ) )
    }
