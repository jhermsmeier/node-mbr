...
class NewLdr extends MBR {
   ...
   * @constant
   */
  static SIGNATURE = 0x4E45574C4452
  ...
  parse( buffer, offset, noAssert = false ) {
    ...
    if( !noAssert && ( this.loaderSignature !== NewLdr.SIGNATURE ) ) {
      throw new Error(
    ...
    this.oem = this.jmp >= 0x01E ? buffer.toString( 'ascii', offset + 24, offset + 6 ) : null
    ...
    if( buffer.readUInt16LE( offset + 0x1AC ) === MBR.AAP.SIGNATURE ) {
      this.aap = this.aap != null ?
        ...
        new MBR.AAP.Record().parse( buffer, offset + 0x1AC, noAssert )
    } else {
      this.aap = null
    }
  ...
  write( buffer, offset ) {
    ...
    buffer[ offset + 1 ] = this.jmp
    buffer.writeUIntBE( this.loaderSignature, offset + 2, 6 )
    buffer[ offset + 8 ] = this.physicalDrive

    this.firstCHS.write( buffer, offset + 9 )

    // Optional fields
    if( this.jmp >= 0x00A ) buffer.writeUInt8( this.minDL, offset + 12 )
    if( this.jmp >= 0x010 ) buffer.writeUIntLE( this.reserved, offset + 13, 3 )
    if( this.jmp >= 0x014 ) buffer.writeUInt32LE( this.firstLBA, offset + 16 )
    if( this.jmp >= 0x016 ) buffer.writeUInt16LE( this.patchOffset, offset + 20 )
    if( this.jmp >= 0x018 ) buffer.writeUInt16LE( this.checksum, offset + 22 )
    if( this.jmp >= 0x01E ) buffer.write( this.oem, offset + 24, 6, 'ascii' )
